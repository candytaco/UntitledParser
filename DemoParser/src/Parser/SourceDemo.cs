...
namespace DemoParser.Parser {
	...
	public class SourceDemo : PrettyClass {
		...
		{
			XElement root = new XElement("DemoFile");
			this.XMLWrite(root);
		...
		{
			parent.Add(new XComment("Generated by untitled parser"));
			if (FileName != null)
				parent.Add(new XElement("File-Name", FileName));
			parent.Add(new XElement("Predicted-Game", DemoInfo.Game));

			Header.XMLWrite(parent);
			foreach (PacketFrame frame in Frames)
				frame.XMLWrite(parent);
			if ((DemoParseResult & DemoParseResult.Success) == 0)
				parent.Add(new XComment("\n\nMore data remaining, parsing failed..."));
			if (ErrorList.Count > 0)
			{
				XElement errors = new XElement("Errors");
				foreach (string s in ErrorList)
					errors.Add(new XElement("Error", s));
				parent.Add(errors);
			}
		}
	}
